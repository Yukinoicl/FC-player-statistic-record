...
Do **not** commit the `.exe` to Git. It would bloat the repo forever, and GitHub also limits large files in git history.

Upload `FC26Career-Setup.exe` as a **GitHub Release** asset instead. After you publish a release, replace this link with your repository:
...
When you build locally, the installer is written to `fcdataserver/dist/FC26Career-Setup.exe`. On GitHub: **Releases → Create a new release → attach that exe**.
...
Frontend and backend live in two folders and are used together.

| Folder | Role |
|--------|------|
| `fcplayerdata` | Vue UI |
| `fcdataserver` | Spring Boot + SQLite |

### Daily development

1. Start the backend (`fcdataserver`) from IDEA, or with Maven. Port **11899**.
...
Static files go to `fcdataserver/src/main/resources/static/`. Start the backend and open `http://127.0.0.1:11899`.
